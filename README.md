...
| `terrain.mjs` | The deterministic terrain rung: a terrain verb plus a terrain noun in one clause compiles to a bounded `terrain_work` action, re-validated through the executor allowlist. Bedrock finds the real ground beneath foliage, snapshots the exact volume, clears or levels it, and supports rollback and undo. |
...
The terrain rung is deterministic, not model-authored. It is placed after the "the child only asked for an explanation" bail, so `just explain how to clear a 50x50 area` still explains rather than digging. Every candidate it authors is re-validated through `allowedWizardAction`, and it returns that gatekeeper's output rather than the object it built. The executor supports an exact footprint up to 64 by 64, finds solid ground beneath logs and leaves, clears up to 32 blocks above it, and in level mode fills holes up to four blocks deep. A persistent world structure backs rollback after interruption and ten-minute `wizard undo`.
...
- The T flip-flop, calculator, command lessons, validated plans, and typed terrain work are transactional and undoable. They honor configured spawn protection and roll back on failure or disconnect.
...
- The never-empty rule is gated on intent, and intent recognition is still lexical. Terrain orders such as `clear a 50x50 area`, `flatten this hill`, `dig out a big hole here`, `make this flat`, and `wiz can you clear the trees around here` compile to typed work. Questions and entangled build requests stay on their own routes.
