...
def patch_bytes(source):
    ...
    if experiments is None:
        body = b"".join(byte_entry(name) for name in FLAGS) + b"\x00"
    ...
    for name in FLAGS:
        ...
        if child is None or child["type"] != 1 or data[child["value"]] != 1:
...
def self_test():
    ...
    payload = named_header(10, "") + experiments + b"\x00"
...
def main():
    if sys.argv[1:] == ["--self-test"]:
        ...
        return
    if len(sys.argv) != 2:
        raise SystemExit("usage: enable-beta-apis.py /path/to/world/level.dat")
    path = Path(sys.argv[1]).resolve()
    original = path.read_bytes()
    patched = patch_bytes(original)
    backup = path.with_suffix(path.suffix + ".pre-beta.bak")
    if backup.exists():
    ...
    print(f"enabled Beta APIs in {path}")
