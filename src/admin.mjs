...
async function loadLogs(){try{$("logs").textContent=(await api("/api/logs")).logs||"No log output yet."}catch(error){$("logs").textContent=error.message}}
...
Promise.all([status(),loadSettings(),loadLogs()]);setInterval(status,5000);
</script>
</body></html>`;
