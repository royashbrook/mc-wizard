...
<body>
<main class="shell">
  <header class="top"><div><h1>MC Wizard<br>operator desk</h1><p>Live controls for the local family server.</p></div><div class="health"><span class="badge" id="bedrock">Bedrock</span><span class="badge" id="brain">Brain</span><span class="badge" id="provider">Provider</span><span class="badge" id="graph">Knowledge graph</span><span class="badge" id="memory">Private notes</span></div></header>
  <div class="grid">
    <div class="stack">
...
const $=id=>document.getElementById(id);async function api(path,options={}){const response=await fetch(path,{headers:{"content-type":"application/json"},...options});const value=await response.json();if(!response.ok)throw new Error(value.error||"Request failed");return value}
  async function status(){const value=await api("/api/status");for(const key of ["bedrock","brain","provider"]){$(key).dataset.ok=String(Boolean(value[key]));}$("provider").textContent=value.providerName||"Provider";const graph=value.graph||{};$("graph").dataset.ok=String(/^kg-\\d+-\\d+-[a-f0-9]+$/i.test(String(graph.revision||"")));$("graph").textContent='Graph: '+(graph.revision||"building");const notes=value.preferences||{};$("memory").dataset.ok=String(Boolean(value.brain));$("memory").textContent='Private notes: '+(Number(notes.preferences)||0)}
...
function formatInteraction(entry){const time=new Date(entry.timestamp).toLocaleTimeString();const who=(entry.playerHash||"unknown").slice(0,10);if(entry.event==="ask")return '['+time+'] '+entry.mode+'/'+who+' > '+(entry.input||'')+'\\n['+time+'] '+(entry.mode==='general'?'AI':'MC Wizard')+' < '+(entry.answer||'')+(entry.actionLabel?'\\n  action: '+entry.actionLabel:'')+(entry.requestId?' · request '+entry.requestId:'');if(entry.event==="feedback")return '['+time+'] grade/'+who+' '+entry.grade+'/5'+(entry.requestId?' · request '+entry.requestId:'')+(entry.goalId?' · goal '+entry.goalId:'')+(entry.note?'\\n  child: '+entry.note:'')+(entry.actionLabel?'\\n  graded: '+entry.actionLabel:'')+(entry.responseMode?' · mode '+entry.responseMode:'')+(entry.followUp?.answer?'\\n  MC Wizard < '+entry.followUp.answer:'')+(entry.followUp?.actionLabel?'\\n  next action: '+entry.followUp.actionLabel:'');return '['+time+'] action/'+who+' '+(entry.outcome?.status||'unknown')+' '+entry.requestId+(entry.detail?' — '+entry.detail:'')+(entry.actionLabel?'\\n  next action: '+entry.actionLabel:'')+(entry.answer?'\\n  MC Wizard < '+entry.answer:'')}
function feedbackSummary(entries){const grades=entries.filter(entry=>entry.event==="feedback"&&Number.isInteger(entry.grade)).map(entry=>entry.grade);if(!grades.length)return'';const counts=[1,2,3,4,5].map(grade=>grade+'★ '+grades.filter(value=>value===grade).length).join(' · ');const average=(grades.reduce((sum,grade)=>sum+grade,0)/grades.length).toFixed(1);return 'Feedback: '+grades.length+' grades · average '+average+'/5 · '+counts}
let interactionsLoading=false;async function loadInteractions(){if(interactionsLoading)return;interactionsLoading=true;try{const panel=$("interactions");const entries=(await api("/api/interactions")).interactions||[];const summary=feedbackSummary(entries);panel.textContent=entries.length?(summary?summary+'\\n\\n':'')+entries.map(formatInteraction).join('\\n\\n'):"No interactions recorded yet.";requestAnimationFrame(()=>{panel.scrollTop=panel.scrollHeight})}catch(error){$("interactions").textContent=error.message}finally{interactionsLoading=false}}
      ...
      if (request.method === "GET" && url.pathname === "/api/status") {
        const [brain, bedrock] = await Promise.all([
          ...
          return send(response, 200, {
            ...
            graph: brain.graph || { revision: "unavailable", documents: 0, nodes: 0, edges: 0 },
